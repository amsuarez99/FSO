...
describe('adding a new blog', () => {
  test('succeeds with all fields present', async () => {
    ...
    const { body: returnedBlog } = await api.post('/api/blogs').send(newBlog).expect(201).expect('Content-Type', /application\/json/)
...
describe('update a blog', () => {
  test('it updates an existing blog', async () => {
    ...
    const { body: returnedBlog } = await api.put(`/api/blogs/${blogToUpdate.id}`).send(updatedBlog).expect(200).expect('Content-Type', /application\/json/)
...
describe('when there is one user in db', () => {
  test('creation fails with an existing username', async () => {
    ...
    const response = await api.post('/api/users').send(newUser).expect(400).expect('Content-type', /application\/json/i)
  ...
  test('creation succeeds with a fresh username', async () => {
    ...
    await api.post('/api/users').send(newUser).expect(201).expect('Content-type', /application\/json/i)
    const usersAtEnd = await helper.usersInDb()
  ...
  test('lists all users', async () => {
    const usersAtStart = await helper.usersInDb()
    const response = await api.get('/api/users').expect(200).expect('Content-type', /application\/json/i)
    ...
    const {body: endUsers} = await api.get('/api/users').expect(200).expect('Content-type', /application\/json/i)
